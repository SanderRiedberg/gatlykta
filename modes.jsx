...
function QuizMode({ t, streets, districtIds, focusDistrict, difficulty, mapStyle, onFinish, onQuit }) {
  ...
  const states = useM(() => { const o = {}; solvedIds.forEach(id => o[id] = 'solved'); missedIds.forEach(id => o[id] = 'hinted'); return o; }, [solvedIds, missedIds]);
  ...
  const handleClick = useC((street) => {
    ...
    if (street.id === target.id) {
      ...
      setFlash({ tone: 'ok', text: t('feedback.correct') });
      ...
      if (tries >= 1) {
        ...
        setFlash({ tone: 'warn', text: t('feedback.try_again') });
      }
    }
  }, [target, solvedIds, missedIds, tries, t, next, bestStreak, streak]);
  ...
  return (
    <div className="game-page">
      ...
      <div className="map-stage">
        ...
        {target && (<div className="quiz-prompt"><div className="eyebrow">{t('quiz.prompt')}</div><div className="q"><em>{target.name}</em>?</div></div>)}
